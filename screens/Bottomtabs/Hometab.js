...
export default class HomeScreen extends Component {
    ...
    getArticles = async (category = 'sports') => {
        this.setState({ isLoading: true });
        axios
            .get(`https://gnews.io/api/v4/top-headlines?category=${category}&lang=en&country=any&max=12&apikey=75cbd76c62f0753219c9aff5102df119`)
            .then(response => {
    ...
    handleSearch = async () => {
      ...
      try {
          const response = await axios.get(
              `https://gnews.io/api/v4/search?q=${encodeURIComponent(searchQuery)}&lang=en&country=any&max=12&apikey=75cbd76c62f0753219c9aff5102df119`
          );
  ...
  handleArticlePress = (article) => {
      const { continueReading } = this.state;
      const updatedContinueReading = [article, ...continueReading.filter(a => a.url !== article.url)].slice(0, 5);
  
      this.setState({ continueReading: updatedContinueReading }, () => {
  ...
  getArticles = async (category) => {
      ...
      try {
          const response = await axios.get(
              `https://gnews.io/api/v4/top-headlines?category=${encodeURIComponent(category)}&lang=en&country=any&max=12&apikey=75cbd76c62f0753219c9aff5102df119`
          );
  ...
  fetchRecommendedArticles = async () => {
      ...
      try {
          const promises = categories.map(category =>
              axios.get(
                  `https://gnews.io/api/v4/top-headlines?category=${encodeURIComponent(category)}&lang=en&country=any&max=4&apikey=75cbd76c62f0753219c9aff5102df119`
              )
          );
